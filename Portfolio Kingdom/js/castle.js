...
castle.add(keepSide);
addBlocker(4.6, -5.2, 3.0);

// warm gate torches flanking the entrance, flickering like real fire
const gateTorches = [];
[-gateGap/2 - 1.0, gateGap/2 + 1.0].forEach(x=>{
  const bracket = new THREE.Mesh(new THREE.BoxGeometry(0.18,0.5,0.18), castleStoneDark);
  bracket.position.set(x, 2.1, half - 0.05);
  castle.add(bracket);
  const flame = new THREE.Mesh(new THREE.ConeGeometry(0.14,0.42,6), new THREE.MeshStandardMaterial({color:0xffb347, emissive:0xff7a1a, emissiveIntensity:1.1, flatShading:true}));
  flame.position.set(x, 2.5, half - 0.05);
  castle.add(flame);
  const glow = makeGlowSprite(3.2, 'rgba(255,170,90,0.55)', 'rgba(255,120,60,0)');
  glow.position.set(x, 2.5, half - 0.05);
  castle.add(glow);
  gateTorches.push({flame, glow, phase: Math.random()*Math.PI*2});
});
...
function updateCastle(delta, t){
  ...
  for(const torch of gateTorches){
    const flick = 0.85 + Math.sin(t*14 + torch.phase)*0.1 + Math.sin(t*27 + torch.phase)*0.05;
    torch.flame.scale.set(flick, 0.9 + Math.sin(t*10+torch.phase)*0.15, flick);
    torch.glow.material.opacity = 0.7 + Math.sin(t*9 + torch.phase)*0.2;
  }
}
