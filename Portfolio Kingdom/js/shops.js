...
SHOPS.forEach((s,i)=>{
  ...
  // solid footprint so the rider stops at the wall instead of riding
  // straight through — kept a little smaller than the building itself
  // so the door-proximity prompt can still trigger right in front of it.
  addBlocker(pos.x, pos.z, 3.7);

  interactTargets.push({
    pos: doorPos,
    radius: 9.5,
    label: "Enter " + s.name,
    action: ()=>openShop(s.key)
  });
});
